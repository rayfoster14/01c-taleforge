...
app.post('/api/register', async function (req, res) {
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        return res.status(500).json({ error: 'User already exists or error occurred' });
      }
...
app.post('/api/login', function (req, res) {
  ...
  db.get('SELECT * FROM users WHERE name = ?', [name], async function (err, row) {
    ...
    // Success
    res.status(200).json({ success: true, userId: row.id });
  });
});
...
app.post('/api/publish', function (req, res) {
  ...
  if (!userId || !flowData) {
    return res.status(400).json({ error: 'Missing user ID or flow data' });
  }
  ...
  db.run(
    ...
    function (err) {
      ...
      res.json({ message: 'Flow published successfully' });
    }
  );
});
...
app.post('/api/generateStory', function (req, res) {
  ...
  if (!emotionList) {
    return res.status(400).json({ error: 'Missing emotion list' });
  }

  const apiKey = process.env.OPEN_AI_APIKEY;
 
  if(!apiKey){
    //No api key found, return this story
    ...
    body: JSON.stringify({
      model: "gpt-3.5-turbo",
      messages: [
        { role: "system", content: "You are a fun story teller, who specalises in simple, light-hearted stories" },
        { role: "user", content: `Generate a short story as a paragraph, use these emotions in order: ${emotionList.join(', ')}` }
      ],
      max_tokens: 1000,
      temperature: 0.7
    })
...
app.get('/api/user-flow/:userId', function (req, res) {
  ...
  db.get('SELECT flow_data FROM users WHERE id = ?', [userId], function (err, row) {
    ...
    if (!row || !row.flow_data) {
      return res.json({ nodes: [], edges: [] }); // No saved flow
    }
    ...
    try {
      ...
      return res.status(500).json({ error: 'Invalid flow data format' });
    }
